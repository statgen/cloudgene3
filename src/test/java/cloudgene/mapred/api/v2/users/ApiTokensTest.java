...
public class ApiTokensTest {
	...
	public void testSubmitWithoutVersion() throws InterruptedException {
		...
		RestAssured
				.given()
				.header(accessToken)
				.when().get("/api/v2/users/testusertoken2/profile")
				.then()
				.statusCode(200)
				.body("username", equalTo("testusertoken2"))
				.body("hasApiToken", equalTo(false))
				.body("password", nullValue());
	...
	private Response submitTestJob(String apiToken) {
		...
		return RestAssured
				...
				.when()
				.post("/api/v2/jobs/submit/all-possible-inputs-private")
				.thenReturn();
	}
	...
	private Response submitTestJobWithoutVersion(String apiToken) {
		...
		return RestAssured
				...
				.multiPart("input-folder", new File("test2.txt"))
				.when()
				.post("/api/v2/jobs/submit/app-version-test")
				.thenReturn();
	}
}
